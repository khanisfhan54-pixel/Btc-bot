...
class BybitAdapter(ExchangeAdapter):
    ...
    def normalize(self,raw,*,local_receive_ts:Optional[int]=None):
        ...
        if route=="orderbook": return [CanonicalOrderBookEvent("BYBIT","orderbook",ts,None,now,bids=tuple((float(p),float(q)) for p,q in d.get("b",[])),asks=tuple((float(p),float(q)) for p,q in d.get("a",[])),update_id=d.get("u"),sequence=d.get("seq"),is_snapshot=raw.get("type")=="snapshot")]
        if route=="trades": return [CanonicalTradeEvent("BYBIT","trades",ts,None,now,trade_id=x.get("i"),price=float(x["p"]),quantity=float(x["v"]),side=x.get("S")) for x in d]
        if route=="ticker":
