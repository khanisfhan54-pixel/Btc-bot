...
def _smoke_rows(n: int = 80) -> List[Dict[str, Any]]:
    ...
    for i in range(n):
        ...
        ts = start + i * interval
        rows.append({
            "symbol": "BTCUSDT", "bar_interval": "5m", "timestamp_ms": ts, "bar_start_ts_ms": ts, "bar_end_ts_ms": ts + interval, "feature_available_ts_ms": ts + interval, "prediction_timestamp_ms": ts + interval,
