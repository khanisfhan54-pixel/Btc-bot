        ...
        if i < lookback - 1 or i + h >= len(samples):
            ...
            for fut in samples[i + 1: i + h + 1]:
                ...
                if hi >= high_pool * (1.0 + buffer) and close <= high_pool:
                    event = True; event_side = "high"; event_ts = fut["timestamp_ms"]; break
                if lo <= low_pool * (1.0 - buffer) and close >= low_pool:
                    event = True; event_side = "low"; event_ts = fut["timestamp_ms"]; break
            label = 1 if event else 0
            reason = event_side or "no_sweep"
        labels.append({"row_index": i, "timestamp_ms": sample["timestamp_ms"], "label": label, "reason": reason, "horizon_bars": h, "feature_available_ts_ms": sample["feature_available_ts_ms"], "event_timestamp_ms": event_ts if label == 1 else None})
