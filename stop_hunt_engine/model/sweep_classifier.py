...
class SweepClassifier:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, *, run_importance_audit: bool = True) -> "SweepClassifier":
        ...
        self.model.fit(X, y); self._classes = self.model.classes_; self._fitted = True
        if run_importance_audit and np.unique(y).size >= 2 and X.shape[0] >= 10:
            self.assert_max_feature_importance(X, y, threshold=self.max_feature_importance)
        return self
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        if not self._fitted or self.model is None: raise RuntimeError("SweepClassifier.predict_proba called before fit()")
        X = np.atleast_2d(np.asarray(X, dtype=float)); proba_all = self.model.predict_proba(X)
