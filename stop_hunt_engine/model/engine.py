...
class SHPEPrediction:
    p_sweep: float; degraded: bool; regime_used: str; stale_dimensions: Tuple[str,...]; model_version: str; raw_p_sweep: Optional[float]=None
DEGRADED_FALLBACK_P=0.5; STALENESS_LIMIT=2
@dataclass
class StopHuntProbabilityEngine:
    ...
    def predict(self,fv: StopHuntFeatureVector)->SHPEPrediction:
        ...
        if len(stale)>self.staleness_limit: return SHPEPrediction(DEGRADED_FALLBACK_P,True,"<degraded>",stale,self.model_version,None)
        x=feature_vector_to_array(fv).reshape(1,-1); regime=fv.regime.regime_label; raw_p,used=self.classifier.predict_proba(x,regime)
