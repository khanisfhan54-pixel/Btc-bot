...
def compute_confluence_score(components: dict) -> float:
    ...
    AUDIT-FIX: callers and downstream invariants must clamp/check against
    [0, 10].  Multiplied by 10.0 at the end and clamped accordingly.
    """
    try:
...
def run_all_engines(
    ...
    try:
        ...
        # AUDIT-FIX: log full traceback + maintain an in-process error counter
        # so operators / harnesses can detect silent fallback drift over time.
        run_all_engines._error_count = int(getattr(run_all_engines, "_error_count", 0)) + 1
        logger.error(
            "run_all_engines error: %s (fallback dict returned, err_count=%d)",
            exc, run_all_engines._error_count, exc_info=True,
        )
        _fallback = {
